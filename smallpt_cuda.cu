...
__device__ double PRNG( int seed) {
    return ((1664525 * seed + 1013904223) % 4294967296) / (double)4294967296;
}

__device__ double PRNG( double seed) {
    return ((1664525 * (int)(4294967296 * seed) + 1013904223) % 4294967296) / (double)4294967296;
}
...
__device__ Vec radiance(const Ray &r, int depth, double sampler, Sphere* spheres)
{
  ...
  { 
    ...
    double r1 = 2 * M_PI * (sampler = PRNG(sampler)), r2 = (sampler = PRNG(sampler)), r2s = sqrt(r2);
    Vec w = nl;
    Vec u = ((fabs(w.x) > .1 ? Vec(0, 1) : Vec(1)) % w).norm();
    Vec v = w % u;
    Vec d = (u * cos(r1) * r2s + v * sin(r1) * r2s + w * sqrt(1 - r2)).norm();
    return obj.e + f.mult(radiance(Ray(x, d), depth, sampler, spheres));
  }
  else if (obj.refl == SPEC) // Ideal SPECULAR reflection
  { 
    return obj.e + f.mult(radiance(Ray(x, r.d - n * 2 * n.dot(r.d)), depth, sampler+1, spheres));
  }
  ...
  if ((cos2t = 1 - nnt * nnt * (1 - ddn * ddn)) < 0) // Total internal reflection
    return obj.e + f.mult(radiance(reflRay, depth, sampler+2, spheres));
  ...
  return obj.e + f.mult(depth > 2 ? ((sampler = PRNG(sampler)) < P ? // Russian roulette
                                         radiance(reflRay, depth, sampler+1, spheres) * RP
                                                     : radiance(Ray(x, tdir), depth, sampler+2, spheres) * TP)
                                  : radiance(reflRay, depth, sampler+3, spheres) * Re + radiance(Ray(x, tdir), depth, sampler+4, spheres) * Tr);
}
...
__global__ void render (int samples, Vec *c, Sphere* spheres)
{
  ...
  {  
    ...
    r = radiance(Ray(cam.o + d * 140, d.norm()), 0, r2, spheres) * (1.0f / samples);

    Vec to_add(0.25 * clamp(r.x), 0.25 * clamp(r.y), 0.25 * clamp(r.z));
    atomicAddVec(&c[id], to_add);atomicAddVec(&c[id], to_add);atomicAddVec(&c[id], to_add);atomicAddVec(&c[id], to_add);
  }
  ...
  for (int sy = 0, i = (IMG_H - y - 1) * IMG_W + x; sy < 2; sy++) { // MXAA rows
    ...
    {      
        for (int s = 0; s < samples; ++ s) {                          // Ray samples per MXAA sample
            ...
            //r = r + radiance(Ray(cam.o + d * 140, d.norm()), 0, r2, spheres) * (1.0f / samples);
            r = radiance(Ray(cam.o + d * 140, d.norm()), 0, r2, spheres) * (1.0f / samples);
            Vec to_add(0.25 * clamp(r.x), 0.25 * clamp(r.y), 0.25 * clamp(r.z));
