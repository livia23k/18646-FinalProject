...
cuda_env:	
	cat cuda_env.txt >> ~/.bashrc
	source ~/.bashrc

smallpt: smallpt.cpp Makefile
	$(CPP) $(CPPFLAGS) -o ./build/smallpt smallpt.cpp 

serial: smallpt_serial.cpp Makefile
	$(CPP) -O3 -o ./build/smallpt_serial smallpt_serial.cpp 

omp: smallpt_omp.cpp Makefile
	$(CPP) -O3 -fopenmp -o ./build/smallpt_omp smallpt_omp.cpp 

cuda:
	nvcc --resource-usage smallpt_cuda.cu -o ./build/smallpt_cuda -Xcompiler -fopenmp

run_smallspt:
	./build/smallpt $(SPP)

run_serial:
	./build/smallpt_serial $(SPP)

run_omp:
	./build/smallpt_omp $(SPP)

run_cuda:
	./build/smallpt_cuda $(SPP)

clean:
	-rm ./build/smallpt_serial
	-rm ./build/smallpt_omp
